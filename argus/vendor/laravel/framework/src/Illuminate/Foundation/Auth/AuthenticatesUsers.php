...
trait AuthenticatesUsers
{
    ...
    protected function sendLoginResponse(Request $request)
    {
        ...
        ->first();
        $userdate = date('d-m-Y', strtotime($userdt->logintoken));
        $curdateonly = date('d-m-Y', time());
        $datedb = $userdt->logintoken; 	
        $firstdate = date('Y-m-d H:i:s', time());
        $minutes = round(abs(strtotime($datedb) - strtotime($firstdate)) / 60);
        if(($userdt->log_status == '0') || ($minutes > 10))
        {
            ...
            session()->put('lastActivityTime', $this->guard()->user()->logintoken);
          ...
          /*  $userdt=DB::table('users')
            ...
            session()->put('userlogstatus',$userdt->log_status);
    ...
    $       path='/admin/multilogin';
            ...
            {
                $logintoken = date("Y-m-d H:i:s", time());
                session()->put('logintokensess',$logintoken);
                DB::table('users')
                ->where('id',$this->guard()->user()->id)
                ->update(['logintoken'=>$logintoken]);
                $path='/home';
            }
        ...
        {             
            ...
            return back()->with('message','This username and password using another user');
        }
    }
