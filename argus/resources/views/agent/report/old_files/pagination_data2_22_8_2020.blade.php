...
echo $agentdt->fname." ".$agentdt->lname;
				 //echo $item->fname; ?></td>
				<td><?php 
/*$hubdt=DB::table('sensor_hubs')->join('sensors','sensors.hub_id','=','sensor_hubs.id')->where('sensor_id',$item->sensor_id)->where('sensor_hubs.hub_id',$item->hub)->where('agent',$agent)->first();
...
//alert("pageno"+pageno);
$.ajax({
			url: "{{ url('/agent/getsensortimefetchpage') }}",
			type: "post",
...
$.ajax({
			url: "{{ url('/agent/getsensortimefetchpageprev') }}",
			type: "post",
			data: $('#tmefrm').serialize(),
			success: function(data)
			{
