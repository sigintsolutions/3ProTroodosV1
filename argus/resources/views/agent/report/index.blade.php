...
<script>
	//Single area chart sensor time report
...
//Push Notification messages count
													...
													<div class="row">													
														<div class="col-md-5">
															<ul class="nav nav-pills">
																...
																<li class="nav-item euz_b greport" style="min-width: 150px;display:none" >
...
<i class="fas fa-chart-line"></i> Graphical Report</button>
														...
														<div class="col-md-7">
															<div class="input-group float-right w-50" >
																...
																<select  style="display:none" name="chartname" required="" class="form-control float-right w-50" id="typegraph">
...
<button class="euz_btn_add euz_pointer float-right ml-3" disabled="disabled" style="background-color:#d6d6d6;border:none;display:none" type="submit" id="exportsensor" target="_blank">EXPORT</button>
													...
													<div class="tab-pane active sl mt-5" id="sensorlist">
														<div id="table_data2">
...
for($i=0;$i<=25;$i++){
?>
<div class="euzscrollbar scrooldiv<?php echo $i;?>" style="width:100%;overflow-x:scroll;overflow-y:hidden;scrollbar-color: #cacaca white;scrollbar-width:thin;display: none;">
<div id="demo<?php echo $i;?>" class="sensdemo"></div></div>
<?php } ?>
										...
										<div class="col-md-12">
											...
											<div class="tab-content euz_border p-3 bg-white">															
												<div class="row">														
													<div class="col-md-12">
														<ul class="nav nav-pills">
															<li class="nav-item1 euz_b1" style="min-width: 150px;">
																<a id="send" class="sd nav-link active"  href="#sensordata"><i class="fas fa-table"></i> Report</a>
															</li>
															<li class="nav-item1 euz_b1" style="min-width: 150px;">
																
																<a id="gr1" onclick="graph()" class="chb w-100 border-0 nav-link"  href="#carthub">
																	<i class="fas fa-chart-line"></i> Graphical Report
																</a>
...
<div  id="carthub" style="">

<?php
//for($i=0;$i<=25;$i++){
?>
<!--<div id="hb<?php //echo $i;?>" style="width:100%;overflow-x:scroll;scrollbar-color:white;scrollbar-width:thin;">
<div id="demohub<?php //echo $i;?>" class="demohub" style="width:5000px;overflow-x:scroll;overflow-y:hidden;"></div>
...
<?php //} ?>
	...
	<div class="row">
		<div class="col-md-12 mt-3">
			<div class="">
	<div class="col-md-12 px-0 chart1old">																<div class="loaderhub" style="display: none; text-align: center;"><img src="{{ url('/loader/2.gif') }}" /></div>
<div class="loaderhub1" style="display: none; text-align: center;"><img src="{{ url('/loader/2.gif') }}" /></div>																<div id="chartajaxhub"></div>
...
//Search with Sensor Time Report
...
//Deleting sensor time report temp data in temp table
$.ajax(
        {
            url: "{{ url('/agent/deletetempdata') }}",
             type: "get",
             data:{},
            success: function(data)
             {

//storing in temp data for chart generating sensor time report
...
//Showing table and chart div show and hide for hub
...
//Showing table and chart div show and hide for Sensor Time Report
	$(".fe").click(function(){
					...
					var nameArr = jsonData.value[k][0].split(',');
							...
							if(nameArr.length < 50)
							{
								var options = 
								{
									...
									vAxis: 
									{ 
										title:unit,titleTextStyle: { fontSize:15 },
										textStyle : 
										{
											fontSize:15
										} 
							...
							else if(nameArr.length < 1800)
							{
								...
								$(".sensdemo").width("27000px");
							}
							else if(nameArr.length < 2500)
							{
								var options = 
								{
									title : title,is3D: true,titleTextStyle: {fontSize:15},
...
data[k].addRow([nameArr1[m],parseFloat(nameArr[m])]);
...
$("#cart").show();
						var chart = new google.visualization.ColumnChart(document.getElementById('demo'+k))
						chart.draw(data[k], options);
...
$("#cart").show();
						var chart = new google.visualization.ComboChart(document.getElementById('demo'+k))
						chart.draw(data[k], options);
...
$("#cart").show();
						var chart = new google.visualization.ScatterChart(document.getElementById('demo'+k))
						chart.draw(data[k], options);
...
$("#cart").show();
						var chart = new google.visualization.SteppedAreaChart(document.getElementById('demo'+k))
						chart.draw(data[k], options);
...
$("#cart").show();
						var chart = new google.visualization.LineChart(document.getElementById('demo'+k))
						chart.draw(data[k], options);
...
var chart = new google.visualization.AreaChart(document.getElementById('demo'+k));
					...
					var chart = new google.visualization.BarChart(document.getElementById('demo'+k));
...
var chart = new google.visualization.PieChart(document.getElementById('demo'+k));
...
var chart = new google.visualization.ScatterChart(document.getElementById('demo'+k));
						...
						for (var i = 0; i < jsonData.value[k].length; i++) 
					{
						...
						var chart = new google.visualization.Histogram(document.getElementById('demo'+k));
					}
					if(val == 'LineChart')
					{
						data[k].addColumn('string', 'Date');
...
$("#carthub").show();
//var chart = new google.visualization.AreaChart(document.getElementById('demohub'+k));
var chart = new google.visualization.AreaChart(document.getElementById('hubchart'+k));
...
for(m=0;m<nameArrcolmn.length;m++){
	//alert(nameArr1[m],nameArr[m]);
//data[k].addRow([nameArr1[0],parseInt(nameArr[0])]);
data[k].addRow([nameArr1colmn[m],parseFloat(nameArrcolmn[m])]);
}
$("#carthub").show();
//var chart = new google.visualization.ColumnChart(document.getElementById('demohub'+k));
...
for(m=0;m<nameArrcomb.length;m++){
	//alert(nameArr1[m],nameArr[m]);
//data[k].addRow([nameArr1[0],parseInt(nameArr[0])]);
data[k].addRow([nameArr1comb[m],parseFloat(nameArrcomb[m])]);
}
$("#carthub").show();
//var chart = new google.visualization.ComboChart(document.getElementById('demohub'+k));
	var chart = new google.visualization.ComboChart(document.getElementById('hubchart'+k));
...
for(m=0;m<nameArrscat.length;m++){
	//alert(nameArr1[m],nameArr[m]);
//data[k].addRow([nameArr1[0],parseInt(nameArr[0])]);
data[k].addRow([nameArr1scat[m],parseFloat(nameArrscat[m])]);
}
$("#carthub").show();
var chart = new google.visualization.ScatterChart(document.getElementById('hubchart'+k));

//var chart = new google.visualization.ScatterChart(document.getElementById('demohub'+k));
...
data[k].addRow([nameArr1step[m],parseFloat(nameArrstep[m])]);
}
	//

$("#carthub").show();
var chart = new google.visualization.SteppedAreaChart(document.getElementById('hubchart'+k));
	//var chart = new google.visualization.SteppedAreaChart(document.getElementById('demohub'+k));
chart.draw(data[k], options);
}
...
for(m=0;m<nameArrline.length;m++){
	//alert(nameArr1line[m],nameArrline[m]);
//data[k].addRow([nameArr1[0],parseInt(nameArr[0])]);
data[k].addRow([nameArr1line[m],parseFloat(nameArrline[m])]);
}
$("#carthub").show();
var chart = new google.visualization.LineChart(document.getElementById('hubchart'+k));

//var chart = new google.visualization.LineChart(document.getElementById('demohub'+k));
chart.draw(data[k], options);
