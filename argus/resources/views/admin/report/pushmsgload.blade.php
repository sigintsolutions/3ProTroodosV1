...
$agentdt=DB::table('users')->where('id',$agentid)->first();


    ?>
                <tr id="tr<?php echo $message->udi;?>" >
                    <td <?php if ($message->readflag==0){?> class="grcolor" <?php } ?> ><?php echo $i; //echo $message->readflag;?></td>
                    <td <?php if ($message->readflag==0){?> class="grcolor" <?php } ?>>{{ $agentdt->fname }} {{ $agentdt->lname }}</td>
...
<?php 
$ald=$message->algorithmid;
$msgdt=DB::table('algorithm')->where('id',$ald)->first();
echo $msgdt->push_message;
?>
