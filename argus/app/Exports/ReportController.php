...
->select('users.fname', 'users.lname','userdatamessages.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessages.readflag','userdatamessages.userdatamsgid as udi','userdatamessages.*')->paginate(100);
...
$currentoffset=0;
$pageno=0;
		return view('admin.report.index')->with(['sensors'=>$sensors,'agents'=>$agents,'agents_count'=>$agents_count,'sensors_count'=>$sensors_count,'perlogin'=>$perlogin,'perlogout'=>$perlogout,'perlogin2'=>$perlogin2,'perlogout2'=>$perlogout2,'groups'=>$groups,'hubs'=>$hubs,'messages'=>$items,'countmsg'=>$countmsg,'currentoffset'=>$currentoffset,'pageno'=>$pageno]);
	}
...
$groups=DB::table('dbo_payloadercharttemp')->where('dbo_payloadercharttemp.unit', $unit)->whereIn('dbo_payloadercharttemp.hub', $hubsname)->whereRaw('dbo_payloadercharttemp.time between "'.$start_month.'" and "'.$end_month.'"')->where('loginid',$loginid)->whereIn('dbo_payloadercharttemp.sensor_id', $sensor)->get();
...
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_month.'" and "'.$end_month.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->get();
foreach ($items as $item) {
	$data=array("loginid"=>$loginid,"hub"=>$item->hub,"time"=>$item->time,"sensor_id"=>$item->sensor_id,"unit"=>$item->unit,"value"=>$item->value,"sensor_type"=>$item->sensor_type,"utc"=>$item->utc);
...
$currentoffset=$pageno*1000;
$skip=$currentoffset;
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_month.'" and "'.$end_month.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip($skip)->take(1000)->get();
$pageno=$pageno+1;
...
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_week.'" and "'.$end_week.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip(0)->take(1000)->get();
		}
		if($tme =='day')
		{
			/*$items = DB::table('sensors')
...
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereDate('dbo_payloader.time', Carbon::today())->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip(0)->take(1000)->get();			
		}
		if($tme =='month')
		{
			$start_month = strtotime("-29 day");
...
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_month.'" and "'.$end_month.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip(0)->take(1000)->get();


$skip1=0;
$limit1=100;
$arrids[]=array();
//$totcount=$items=DB::table('dbo_payloader')->count();
//$nooftimes1=($totcount/100);
$nooftimes=1;
$i=0;
$currentoffset=0;
$pageno=1;
...
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start.'" and "'.$end.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip(0)->take(1000)->get();
...
/*public function getpushnotmsgandor_old(){
$getdata=DB::table('dbo_payloader')->where('processedflagall',0)->take(1)->get();
foreach ($getdata as $gd) {
	$value=$gd->value;
	$sensorname=$gd->sensor_id;
...
$allcond=DB::table('algorithm_sensor')->where('algorithm_id',$nd->algorithm_id)->get();
$noofconditions=count($allcond);
if ($noofconditions==2){
foreach ($allcond as $ac) {
	$condfirst=$ac->condition2;
...
$maxvalue=$nd->max_value;
$algid=$nd->algorithm_id;
if ((($value >= $nd->min_value) && ($value <= $nd->max_value))){
		$algdata=DB::table('algorithm')->where('id',$algid)->first();
		$userid=$algdata->userid;
		$data=array('userid'=>$userid,'algorithmid'=>$algid,'payloaderid'=>$gd->id);
$insertmsg=DB::table('userdatamessages')->insert($data);
