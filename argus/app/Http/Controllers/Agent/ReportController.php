...
//dd("hello");	
	...
	public function uppushmsgreadflagcount(){
		//Fetching push notification message


		$countmsgdt=DB::table('userdatamessagesagent')->where('userid',session()->get('userid'))->where('readflaguserid',0)->get();
				$countmsg=count($countmsgdt);
				echo $countmsg;
	}
	public function uppushmsgreadflag()
	{
		...
		//Updating count of messages read
		$msgid=$_GET['msgid'];
		if($msgid == 'all')
		{
			$upread = DB::table('userdatamessagesagent')->where('userid',session()->get('userid'))->where('readflaguserid', '0')->update(['readflaguserid'=>1]);
		}
		else
		{
			$upread = DB::table('userdatamessagesagent')->where('userdatamsgid',$msgid)->update(['readflaguserid'=>1]);
		}
		echo "1";
	}
...
$groups=DB::table('dbo_payloadercharttemp as db')->where('loginid',$loginid)->select('db.value','db.utc','db.sensor_id','db.hub','db.hub_sensorid','db.agentname','db.gatewaygrp')->orderby('id','asc')->get();
...
//Displaying in sensor table previous
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_week.'" and "'.$end_week.'"')->orderby('db.id','desc')->skip($skip)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereDate('db.utc', Carbon::today())->orderby('db.id','desc')->skip($skip)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_month.'" and "'.$end_month.'"')->orderby('db.id','desc')->skip($skip)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start.'" and "'.$end.'"')->orderby('db.id','desc')->skip($skip)->select('db.*');
...
//Fetching in sensor table next button
//$agent = $request->input('agent');
		$agent = session()->get('userid');
		$group = $request->input('group');
		$hub = $request->input('hub');
...
//Saving in Temporary table for Sensor Time Report for chart
//$agent = $request->input('agent');
	$agent = session()->get('userid');
		$group = $request->input('group');
		$hub = $request->input('hub');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_week.'" and "'.$end_week.'"')->orderby('db.id','desc')->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereDate('db.utc', Carbon::today())->orderby('db.id','desc')->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_month.'" and "'.$end_month.'"')->orderby('db.id','desc')->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start.'" and "'.$end.'"')->orderby('db.id','desc')->select('db.*');
...
//Displaying in sensor time report
		//$unit="";
		//$agent = $request->input('agent');
		$agent = session()->get('userid');
		$group = $request->input('group');
...
//DB::enableQueryLog();
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_week.'" and "'.$end_week.'"')->orderby('db.id','desc')->skip(0)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereDate('db.utc', Carbon::today())->orderby('db.id','desc')->skip(0)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start_month.'" and "'.$end_month.'"')->orderby('db.id','desc')->skip(0)->select('db.*');
...
$query=DB::table('dbo_payloader as db')->where('db.unit', $unit)->whereRaw('db.utc between "'.$start.'" and "'.$end.'"')->orderby('db.id','desc')->skip(0)->select('db.*');
...
//push not clear while clicking reset button
$items = DB::table('algorithm')->join('userdatamessagesagent', 'userdatamessagesagent.algorithmid', '=', 'algorithm.id')->join('users', 'users.id', '=', 'userdatamessagesagent.userid')->orderBy('userdatamsgid','desc')->where('users.id', session()->get('userid'))->where('users.id', session()->get('userid'))->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);	
...
$items = DB::table('algorithm')->join('userdatamessagesagent', 'userdatamessagesagent.algorithmid', '=', 'algorithm.id')->join('users', 'users.id', '=', 'userdatamessagesagent.userid')->orderBy('userdatamsgid','desc')->where('users.id', session()->get('userid'))->whereRaw('userdatamessagesagent.created_at between "'.$start.'" and "'.$end.'"')->where('users.id', session()->get('userid'))->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);		



		return view('agent.report.pushmsgser')->with(['messages'=>$items]);
	}
...
//Excel export sensor time report
		$agent = session()->get('userid');
		$group = $request->input('group');
		$hub = $request->input('hub');
...
return Excel::download(new ReportExport(array('agent'=>$agent,'group'=>$group,'hub'=>$hubsname,'sensor'=>$sensor,'tme'=>$tme,'from'=>$from,'to'=>$to,'unit'=>$unit,'loginid'=>$loginid)), 'sensortimereport.xlsx');
	...
	function exporthub(Request $request)
    {
		...
		$from = $request->input('from');
		$to = $request->input('to');
		return Excel::download(new Reporthub(array('agent'=>$agent,'group'=>$group,'hub'=>$hubsname, 'tme'=>$tme,'from'=>$from,'to'=>$to,'hubid'=>$hub)), 'sensors.xlsx');		
    }
...
//Getting push messages
			$items = DB::table('algorithm')->join('userdatamessagesagent', 'userdatamessagesagent.algorithmid', '=', 'algorithm.id')->join('users', 'users.id', '=', 'userdatamessagesagent.userid')->orderBy('userdatamsgid','desc')->where('users.id', session()->get('userid'))->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);		
