<?php
    ...
    class AgentsController extends Controller
    {
    	public function index()
        {
            ...
            //Fetching user agent and getting emailid
    	...
    	public function insertAgent(Request $request)
    	{
    		...
    		$service_expiry = $request->input('service_expiry');
...
//Fetching from sensor hubs
    		...
    		->select('sensor_hubs.*', 'hubs.name as hubname', 'sensor_groups.name as groupname', 'sensor_hubs.id as hubid')
    		->first();
    		echo json_encode($hubs);
    	}
    	public function editSensorpoptree($id=0)
    	{
    		...
    		->join('hubs', 'hubs.id', '=', 'sensor_hubs.hub_id')
    		->where('sensors.id', $id)
    		->select('sensors.*', 'hubs.name as hubname', 'sensor_groups.name as groupname', 'sensors.id as sensorid', 'sensors.hub_id as hubid')
    		->first();
    		echo json_encode($hubs);
    	}
...
//Adding in sensor hubs
            ...
            {				
    			...
    			{			
					...
					$hubcnt = DB::table('sensor_hubs')->where('hub_id', $hub_id)->where('group_id', $group_id)->get();
					if(count($hubcnt) == '0')
					{
        				$insert_data = array(
        					'sensor_hub_id'     => $cusid,
...
//Fetching Sensors
    		...
    		$hubname = DB::select('select * from hubs where id ='.$hub[0]->hub_id);
    		//$sensors = DB::table('sensors')->where('hub_id', $hubid)->orderBy('created_at', 'DESC')->get();
    		...
    		->select('sensors.*','sensors.id as sensorid', 'types.name as typename', 'types.brand', 'types.unit')
    		->get();
    		$types = DB::table('types')->get();
    		//$brands = DB::table('brands')->get();
...
//Adding in types and sensors table using excel
            ...
            {				
    			foreach($value as $row)
    			{
    				...
    				//$brand = DB::table('brands')->where('name', $row[2])->get();
    				//$unit = DB::table('measurement_units')->where('minimum', $row[3])->where('maximum', $row[4])->get();
    				//echo count($type);die();
    				if(count($type) == '0')
					{
						$insert_data = array(
...
//Adding sensors
    	...
    	public function editSensorpop($id=0)
    	{
            ...
            //Getting from sensors table
    		$user = DB::table('sensors')
    		->join('types', 'types.id', '=', 'sensors.type')
    		->where('sensors.id', '=', $id)
    		->select('sensors.*', 'sensors.id as id', 'types.*')
    		->first();
    		echo json_encode($user);
    	}
        ...
        public function gethublist(Request $request)
    	{
    		...
    		->select('hubs.name','sensor_hubs.id as hid','sensor_hubs.*')
    		->get();
    	...
    	function editgraph($id=0)
    	{
    		$data = DB::table('sensors')
				...
				->where('sensors.id', '=', $id)
				->select('sensor_graph.*', 'types.*', 'sensor_graph.id as graphid', 'types.min as mingraph', 'types.max as maxgraph')
				->first();
    	...
    	function profileimport(Request $request)
    	{
            ...
            {				
    			foreach($value as $row)
    			{
    				...
    				$userscusid = DB::table('users')->where('customer_id', $row[13])->get();
...
//Editing gateway groups
    		...
    		return back()->with('flash_message', 'Group update successfully.');
    	}
    	function hubeditimport(Request $request)
    	{

//Editing in Hubs table
     				...
     				if(count($type) == '0')
					{
    					...
    					if(!empty($insert_data))
    					{
    						...
    						if($ins)
							{
								$sengroupsins = DB::table('types')->where('sname', $row[1])->get();
								$type_id = $sengroupsins[0]->id;
								$userscusid = DB::table('sensors')->where('id', $row[9])->get();
                				if(count($userscusid) == '0')
                				{
                			        $hub_id = $request->input('hub_id');
                		            $data = array(
    	...
    	public function insertsensorpop(Request $request)
    	{
    		...
    		if(count($group) == '0') 
    		{
    			$data=array('name'=>$name, 'brand'=>$brand, 'unit'=>$unit, 'sname'=>$sname, 'modal'=>$modal, 'min'=>$min, 'max'=>$max, 'remark'=>$remark);
