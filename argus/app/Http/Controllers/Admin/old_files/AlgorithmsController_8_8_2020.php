...
//$groups1[]=array(10,20,30);
$sensorsintbl=DB::table('sensorvalues')->select('sensorname')->groupBy('sensorname')->get();
$sensorunit1=array();
$unit1=array();
$chart1=array();
...
$countdatasensors=count($sensorunit1);
				...
				{													//dd($noofselectedsensors);
					...
					//for($h=0;$h<$noofselectedsensors;$h++)
					{
						...
						//for($m=0;$m<count($groups1);$m++)
						//{
		$arrstringval=array();
		$arrstringhour=array();				
						//if ($h==0){

//$getsensdt=DB::table('sensorvalues1')->where('sensorname', $sensorunit[$h])->get();

	$getsensdt=DB::table('sensorvalues')->where('sensorname', $sensorunit1[$h])->get();	
...
if($tme =='month')
			{
				...
				->join('sensor_groups', 'sensor_groups.id', '=', 'gateway_groups.sensor_group_id')
				->join('sensors', 'sensors.sensor_id', '=', 'dbo_payloadercharttemphub.sensor_id')		
				->whereRaw('sensor_hubs.agent="'.$agent.'"')
				->whereRaw('gateway_groups.id = "'.$group.'"')
				->where('dbo_payloadercharttemphub.hub', $hubsname)						
				->whereRaw('dbo_payloadercharttemphub.time between "'.$start_month.'" and "'.$end_month.'"')
				->orderBy('dbo_payloadercharttemphub.sensor_id')		
				->select('dbo_payloadercharttemphub.*')
				->get();*/
//\DB::connection()->enableQueryLog();
