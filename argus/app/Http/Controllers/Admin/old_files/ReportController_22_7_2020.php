...
foreach ($items as $item) {
	$data=array("loginid"=>$loginid,"hub"=>$item->hub,"time"=>$item->time,"sensor_id"=>$item->sensor_id,"unit"=>$item->unit,"value"=>$item->value,"sensor_type"=>$item->sensor_type,"utc"=>$item->utc,"agentname"=>$agentname);
...
$skip=$currentoffset;
$items=DB::table('dbo_payloader')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_week.'" and "'.$end_week.'"')->where('agent','like',"%$agent%")->whereIn('dbo_payloader.sensor_id', $sensor)->orderby('dbo_payloader.id','desc')->skip(0)->take(100)->get();
//->where('agent','like',"%$agent%")->
$start_month=$start_week;
$end_month=$end_week;
