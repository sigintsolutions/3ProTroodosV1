...
$gatewaygrp=$grpdt->name;
	//$gatewayname
$hubsens=$item->hub."_".$item->sensor_id;
...
//dd($sensors,$sensorshubs);
		...
		{	
			...
			->get();*/
//DB::enableQueryLog();
/*$items=DB::table('dbo_payloader')->join('sensors', 'sensors.sensor_id', '=', 'dbo_payloader.sensor_id')->join('sensor_hubs', 'sensors.hub_id', '=', 'sensor_hubs.id')->where('dbo_payloader.unit', $unit)->whereIn('dbo_payloader.hub', $hubsname)->whereRaw('dbo_payloader.time between "'.$start_week.'" and "'.$end_week.'"')->whereIn('dbo_payloader.sensor_id', $sensor)->where('sensor_hubs.agent',$agent)->where('sensors.agent',$agent)->whereIn('sensors.group_id',$group)->orderby('dbo_payloader.id','desc')->skip(0)->select('dbo_payloader.*','sensors.agent')->take(100)->get();*/
//$hublist=array('BC:DD:C2:CF:A4:EC','24:6F:28:F6:43:5C','BC:DD:C2:CF:A4:EC');
//$senlist=array('SMT100#1-2','BME280#1','BME280#1');
/*$items=DB::table('dbo_payloader')->join('sensors', 'sensors.sensor_id', '=', 'dbo_payloader.sensor_id')->join('sensor_hubs', 'sensors.hub_id', '=', 'sensor_hubs.id')->where('dbo_payloader.unit', $unit)->whereRaw('dbo_payloader.time between "'.$start_week.'" and "'.$end_week.'"')->where('sensor_hubs.agent',$agent)->where('sensors.agent',$agent)->whereIn('sensors.group_id',$group)->whereIn('dbo_payloader.sensor_id', $senlist)->whereIn('dbo_payloader.hub', $hublist)->orderby('dbo_payloader.id','desc')->skip(0)->select('dbo_payloader.*','sensors.agent')->take(100)->get();*/

/*
DB::enableQueryLog();

$query=DB::table('dbo_payloader as db')->join('sensors', 'sensors.sensor_id', '=', 'db.sensor_id')->join('sensor_hubs', 'sensors.hub_id', '=', 'sensor_hubs.id')->where('db.unit', $unit)->whereRaw('db.time between "'.$start_week.'" and "'.$end_week.'"')->where('sensor_hubs.agent',$agent)->where('sensors.agent',$agent)->whereIn('sensors.group_id',$group)->orderby('db.id','desc')->skip(0)->select('db.*','sensors.agent');
for($i=0;$i<count($hublist);$i++){
	if($i==0){
$query->where('sensor_hubs.hub_id',$hublist[$i])->where('sensors.sensor_id',$senlist[$i]);
}
else{
$query->orwhere('sensor_hubs.hub_id',$hublist[$i])->where('sensors.sensor_id',$senlist[$i]);
}
}
