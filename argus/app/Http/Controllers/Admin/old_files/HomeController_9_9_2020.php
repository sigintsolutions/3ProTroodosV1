...
// 		$perlogout=(count($logoutagents)/$total)*100;
		...
		->paginate(15);
		//$logs=array();
		//$agentlist=DB::table('users')->orderby('id', 'asc')->get();


		//return view('home')->with(['logs' => $logs]);
		//$sensors = DB::table('dbo_payloader')->orderby('id', 'desc')->paginate(100);
		$sensors = DB::table('dbo_payloader')->orderby('id', 'desc')->take(100)->select('utc','hub','sensor_id','value')->get();
...
//dd("helloh");
		...
		->first();
        return view('home')->with(['agents_count'=>$agents_count,'agents'=>$agents,'sensors_count'=>$sensors_count,'groups_count'=>$groups_count,'hubs_count'=>$hubs_count,'perlogin'=>$perlogin,'perlogout'=>$perlogout,'loginagents'=>count($loginagents),'logoutagents'=>count($logoutagents),'logs'=> $logs,'sensors'=> $sensors,'countmsg'=>$countmsg,'list'=>$list]);
	}
	public function searchlog(Request $request)
    {
		...
		->select('weather.*', 'weather.id as id', 'loc.loc', 'loc.template')
		->first();
        return view('home')->with(['agents_count'=>$agents_count,'agents'=>$agents,'sensors_count'=>$sensors_count,'groups_count'=>$groups_count,'hubs_count'=>$hubs_count,'perlogin'=>$perlogin,'perlogout'=>$perlogout,'loginagents'=>count($loginagents),'logoutagents'=>count($logoutagents),'logs'=> $logs,'sensors'=>$sensors, 'list'=>$list]);
	}
