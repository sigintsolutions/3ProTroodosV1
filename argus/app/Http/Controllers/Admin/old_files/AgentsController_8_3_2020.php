...
$maccnt = DB::table('sensor_hubs')->where('mac_id', $row[1])->count();
...
return back()->with('flash_message', 'Excel Data Imported successfully.');
			...
			return back()->with('flash_message', 'Excel Data Imported successfully.Skipped Duplicate Mac ids -'.$dup);
            }
    	}
...
$checksensnameexist=DB::table('sensors')->where('sensor_id',$row[0])->where('agent',$agentid)->count();
...
if ($checksensnameexist==0){
    ...
    $masterexist1.=$row[0];
							...
							$checksensnameexist=DB::table('sensors')->where('agent',$agentid)->where('sensor_id',$row[0])->count();
...
$ins = DB::table('sensors')->where('id', $row[8])->update($insert_data);
           ...
           /* print_r($masterexist);
            foreach ($masterexist as $key => $value) {
               echo $value;
            }
            ...
            //echo $dup1;
            //die;
            //$me=implode(",",array_values($masterexist));
            //$dp=implode(",",array_values($dup));
			return back()->with('flash_message', 'Excel Data Imported successfully.'.'Duplicate Sensors- '.$dup1.', Not in Sensor Master Table- '.$masterexist1);
    	}
