...
$pagenohub=0;
//dd("hai");
		return view('admin.report.sea')->with(['sensors'=>$sensors,'agents'=>$agents,'agents_count'=>$agents_count,'sensors_count'=>$sensors_count,'perlogin'=>$perlogin,'perlogout'=>$perlogout,'perlogin2'=>$perlogin2,'perlogout2'=>$perlogout2,'groups'=>$groups,'hubs'=>$hubs,'messages'=>$items,'countmsg'=>$countmsg,'currentoffset'=>$currentoffset,'pageno'=>$pageno,'currentoffsethub'=>$currentoffsethub,'pagenohub'=>$pagenohub]);
	}
...
return json_encode($arr);
	...
	public function getChart_19_8_2020(Request $request)
	{  
		$agent = $request->input('agent');
		$group = $request->input('group');
		$hub = $request->input('hub');
...
$groups=DB::table('dbo_payloadercharttemp')->where('dbo_payloadercharttemp.unit', $unit)->whereIn('dbo_payloadercharttemp.hub', $hubsname)->whereRaw('dbo_payloadercharttemp.time between "'.$start_week.'" and "'.$end_week.'"')->where('loginid',$loginid)->whereIn('dbo_payloadercharttemp.sensor_id', $sensor)->select(array('dbo_payloadercharttemp.*'))->get();
				...
				if(count($groups)>0)
				{
					foreach($groups as $item)
					{
						...
						for($h=0;$h<$noofselectedsensorshub;$h++)
						{

				if (($item->sensor_id == $sensorshub[$h]) && ($item->hub==$shub[$h]))			
							//if ($item->sensor_id == $sensor[$h])
							{
								$arr['value'][$h][] = $item->value;
								$arr['hour'][$h][] = $item->utc;
					...
					//$arr['count'] = $noofselectedsensors;	
					$arr['count'] = $noofselectedsensorshub;		
				}
			}
		}
		if($tme=='day')
...
$pageno=1;
	...
	function exporthub(Request $request)
    {
		...
		//dd($loginid);
		$loginid=session()->get('userid');
		return Excel::download(new Reportadminhub(array('agent'=>$agent,'group'=>$group,'hub'=>$hubsname, 'tme'=>$tme,'from'=>$from,'to'=>$to,'loginid'=>$loginid,'hubid'=>$hub)), 'sensors.xlsx');		
    }
...
$items = DB::table('algorithm')
						->join('userdatamessagesagent', 'userdatamessagesagent.algorithmid', '=', 'algorithm.id')->join('users', 'users.id', '=', 'userdatamessagesagent.userid')->orderBy('userdatamsgid','desc')	
			->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);
	...
	public function pushmsg(Request $request)
	{
		...
		$items = DB::table('algorithm')
			...
			->whereRaw('userdatamessagesagent.created_at between "'.$start.'" and "'.$end.'"')
			//->select('users.fname', 'users.lname', 'sensor_groups.name', 'sensor_hubs.hub_id', 'sensors.sensor_id', 'userdatamessages.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessages.readflag','userdatamessages.userdatamsgid as udi')->paginate(100);
			->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);

			//->get();
		return view('admin.report.pushmsgser')->with(['messages'=>$items]);
	}


public function getpushmsg(){
		
		$items = DB::table('algorithm')->join('userdatamessagesagent', 'userdatamessagesagent.algorithmid', '=', 'algorithm.id')->join('users', 'users.id', '=', 'userdatamessagesagent.userid')->orderBy('userdatamsgid','desc')->select('users.fname', 'users.lname','userdatamessagesagent.created_at', 'algorithm.push_message', 'algorithm.id as pid','userdatamessagesagent.readflag','userdatamessagesagent.userdatamsgid as udi','userdatamessagesagent.*')->paginate(1000);
...
public function uppushmsgreadflag()
{
	...
	if($msgid == 'all')
	{
		$upread = DB::table('userdatamessagesagent')->where('readflag', '0')->update(['readflag'=>1]);
	}
	else
	{
		$upread = DB::table('userdatamessagesagent')->where('userdatamsgid',$msgid)->update(['readflag'=>1]);
	}
	echo "1";
}
