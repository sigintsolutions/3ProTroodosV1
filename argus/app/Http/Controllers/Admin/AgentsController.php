...
$hubs = DB::table('sensor_hubs')
    	...
    	public function deleteAgent($id=0)
    	{
           ...
           DB::table('userdatamessagesagent')->where('userid', '=', $id)->delete(); 
           
$alglist=DB::table('algorithm')->where('userid', '=', $id)->get();
$algid=array();
foreach($alglist as $al){
$algid[]=$al->id;
}
$as=DB::table('algorithm_sensor')->whereIn('algorithm_id', $algid)->delete();
$al=DB::table('algorithm')->whereIn('id', $algid)->delete();

$table='dbo_payloaderalgorithmtemp'.$id;

$statementdp = "drop table $table";
    $dbdp=DB::statement($statementdp);
    $tableandor='dbo_payloaderalgorithmtempandor'.$id;
$statementandordp = "drop table $tableandor";

$dbandordp=DB::statement($statementandordp);
...
DB::table('userdatamessagesagent')->where('userid', '=', $lists)->delete(); 
           
$alglist=DB::table('algorithm')->where('userid', '=', $lists)->get();
$algid=array();
foreach($alglist as $al){
$algid[]=$al->id;
}
...
$table='dbo_payloaderalgorithmtemp'.$lists;

$statementdp = "drop table $table";
    $dbdp=DB::statement($statementdp);
    $tableandor='dbo_payloaderalgorithmtempandor'.$lists;
$statementandordp = "drop table $tableandor";
...
$hubid=$id;
		...
		public function addsensordrop()
    	{
            ...
            //populating in sensor drop down
			$hubid = request()->val;
...
//populating in sensor drop down
		...
		public function addsensorpopval2($val1 = 0, $val2 = 0)
    	{
            ...
            //Populating in sensor hub table
			$val2=$_GET['sensorid'];
			$hubid=$_GET['hb'];
           /* $hubid =$val1;
			echo $val2;
...
//Importing Hub
    	...
    	public function editgrouppop($id=0)
    	{
            ...
            //Fetching from gateway groups table
    	...
    	public function editSensorhubpop($id=0)
    	{
            ...
            //Fetching from sensor hubs table
...
return view('admin.agents.dashconsole')->with([                
    	...
    	function importgroupname(Request $request)
    	{
            ...
            //Importing in sensor groups using excel
    		$this->validate($request, [
    		'select_file'  => 'required|mimes:xls,xlsx'
    		]);
    		$datas = Excel::toArray(new UsersImport,request()->file('select_file'));
    	...
    	function importtype(Request $request)
    	{
            ...
            //Importing Sensor Type using Excel
    		$this->validate($request, [
    		'select_file'  => 'required|mimes:xls,xlsx'
    		]);
    		$datas = Excel::toArray(new UsersImport,request()->file('select_file'));
...
//getting sensor types
    	...
    	function profileimport(Request $request)
    	{
            ...
            //Adding an agent through excel and algorithm data table creating for each agent run time
    		$this->validate($request, [
    		'select_file'  => 'required|mimes:xls,xlsx'
    		]);
...
$table='dbo_payloaderalgorithmtemp'.$userid[0]->id;

$statement = "create table $table(id int(11) primary key auto_increment,utc varchar(200),hub varchar(255),sensor_id varchar(200),sensor_type varchar(200),value varchar(200),unit text,time timestamp,processedflag int(11),processedflagall int(5));";
    $db=DB::statement($statement);

$tableandor='dbo_payloaderalgorithmtempandor'.$userid[0]->id;
...
//Inserting into sensor groups using excel
...
//Inserting into sensor hubs using excel
...
//             }
    	...
    	{
			//Inserting into Sensor Types table using excel
    		$this->validate($request, [
    		'select_file'  => 'required|mimes:xls,xlsx'
    		]);
    		$datas = Excel::toArray(new UsersImport,request()->file('select_file'));
...
//Getting data from weather and loc table
    	...
    	function deleteloc()
    	{
            //Delete location and weather table
...
//Adding in weather table
    	...
    	{
			//echo $_GET['val'];die();
            //Fetching from sensordata table
...
//Grouping Sensordata on the basis of unit
$measureunits = DB::table('sensordata')->groupBy('unit')->get();
